...
public class Hand {
	...
	public void EvalHand() {
		...
		// Four of a Kind
		// Look at cards one, two, three, four
		if (CardsInHand.get(eCardNo.FirstCard.getCardNo()).getRank() == CardsInHand
				.get(eCardNo.FourthCard.getCardNo()).getRank()) {
		...
		// Look at cards two, three, four, five
		...
		if (CardsInHand.get(eCardNo.FirstCard.getCardNo()).getSuit() == CardsInHand
			...
			ScoreHand(eHandStrength.Flush,
					CardsInHand.get(eCardNo.FirstCard.getCardNo()).getSuit()
							.getSuit(), 0, 0);
		}
		
		// Straight
		// If straight is true but not flush
		...
		// Check if pairs are card one, two and three, four
		if (CardsInHand.get(eCardNo.FirstCard.getCardNo()).getRank() == 
				CardsInHand.get(eCardNo.SecondCard.getCardNo()).getRank() && CardsInHand.get
				(eCardNo.ThirdCard.getCardNo()).getRank() == CardsInHand
						.get(eCardNo.FourthCard.getCardNo()).getRank()) {
			ScoreHand(eHandStrength.TwoPair,
					CardsInHand.get(eCardNo.FourthCard.getCardNo()).getRank()
							.getRank(), 0, 1);
		}
		// Check if pairs are card two, three and four, five
		if (CardsInHand.get(eCardNo.SecondCard.getCardNo()).getRank() == 
				CardsInHand.get(eCardNo.ThirdCard.getCardNo()).getRank() && CardsInHand.get
				(eCardNo.FourthCard.getCardNo()).getRank() == CardsInHand
						.get(eCardNo.FifthCard.getCardNo()).getRank()) {
			ScoreHand(eHandStrength.TwoPair,
					CardsInHand.get(eCardNo.FourthCard.getCardNo()).getRank()
							.getRank(), 0, 1);
